...
import API, { BASE_URL } from "../../api/axios"; // 👈 shared instance
import "./Banners.css";
import {
  Search,
  Filter,
...
const Banners = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      // 👇 no hardcoded URL — API instance already has baseURL
      const { data } = await API.get("/banners");

      // handle both raw array and { banners: [...] } shape
  ...
  const handleSaveBanner = async (e) => {
    ...
    data.append(
      "target",
      formData.product || formData.linkUrl || "Custom Target"
    );
    ...
    try {
      // ⚠️ Do NOT set Content-Type manually — your interceptor
      // already strips it for FormData so axios sets the boundary.
      if (formData.id) {
        await API.put(`/banners/${formData.id}`, data);
      } else {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await API.patch(`/banners/${id}/status`, { status: newStatus });
      setBannersList((prev) =>
  ...
  const handleEdit = (banner) => {
    setFormData({
      ...
      bannerUrl: banner.bannerUrl ? `${BASE_URL}${banner.bannerUrl}` : null,
      imageFile: null,
    });
    setActiveDropdownId(null);
  };
  ...
  const activeBannersCount = bannersList.filter(
    (b) => b.status === "Active"
  ).length;
  const inactiveBannersCount = bannersList.filter(
    (b) => b.status === "Inactive"
  ).length;

  // Filter Logic (guarded against missing fields)
  const filteredBanners = bannersList.filter((b) => {
    const q = searchQuery.toLowerCase();
    const titleMatch = String(b.title || "").toLowerCase().includes(q);
    const targetMatch = String(b.target || "").toLowerCase().includes(q);
    const matchesSearch = !q || titleMatch || targetMatch;
            ...
            <form onSubmit={handleSaveBanner} className="banner-form">
                ...
                >
                  {formData.bannerUrl ? (
                    ...
                    <div className="banner-upload-content">
                      ...
                      <p className="banner-upload-title">
                        Click to upload banner image
                      </p>
                      <span className="banner-upload-meta">
              ...
              <div className="banner-form-group">
                ...
                <div className="banner-input-count-wrapper">
                  ...
                  />
                  <span className="banner-char-count">
          ...
          <div className="banner-card banner-list-section">
            ...
            <div className="banner-table-wrapper">
              <table className="banner-table">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan="8" className="banner-no-data">
                        Loading banners…
                      </td>
                    </tr>
                  ) : error ? (
                    ...
                    currentBanners.map((item, index) => (
                      <tr key={item._id}>
                        ...
                        <td>
                          <div
                            className="banner-thumb-box"
                            style={{ background: item.bgGradient }}
                          >
                            {item.bannerUrl ? (
                              <img
                                src={`${BASE_URL}${item.bannerUrl}`}
                                alt={item.title}
                        ...
                        <td>
                          <span
                            className={`banner-link-pill ${
                              item.linkType === "Custom URL" ? "blue" : "green"
                            }`}
                          >
                            {item.linkType}
                          </span>
            ...
            <div className="banner-pagination-wrapper">
              <span className="banner-entries-info">
                ...
                {Math.min(startIndex + ITEMS_PER_PAGE, totalEntries)} of{" "}
