...
const formatReview = (review) => {
  ...
  const customerEmail =
    review?.reviewerEmail || review?.user?.email || "Guest customer";

  const productName = review?.product?.productName || "Product";

  const productSku = review?.product?.sku || "SKU: N/A";
...
const ReviewsManagement = () => {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const formattedReviews = Array.isArray(reviewsData)
        ? reviewsData.map(formatReview)
        : [];
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Failed to fetch reviews.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviews.filter((item) => {
      ...
      const reviewTitle = item?.title?.toLowerCase() || "";
  ...
  const handleExport = () => {
    if (reviews.length === 0) {
      Swal.fire({
        icon: "info",
        title: "No Reviews",
        text: "There are no reviews to export.",
      });
    ...
    Swal.fire({
      ...
      text: "Reviews exported successfully.",
      timer: 1800,
  ...
  const handleView = async (row) => {
    await Swal.fire({
        ...
        <div class="ReviewsManagement-viewReview">
          ...
          <div class="ReviewsManagement-viewRow">
            ...
            <strong class="ReviewsManagement-viewStars">
              ${"★".repeat(row.rating)}
              ${"☆".repeat(Math.max(0, 5 - row.rating))}
            </strong>
          </div>
      ...
      customClass: {
        popup: "ReviewsManagement-swalPopup",
        confirmButton: "ReviewsManagement-swalConfirm",
      },
    });
  };
  ...
  const handlePublish = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Failed to publish review.",
      });
    } finally {
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Failed to reject review.",
      });
    } finally {
      setActionLoading(null);
    }
  ...
  const handleUnpublish = async (id) => {
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to unpublish this review?",
      showCancelButton: true,
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Unpublished",
        text: "Review unpublished successfully.",
        timer: 1500,
        showConfirmButton: false,
      });
    } catch (error) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Failed to unpublish review.",
      });
    } finally {
      setActionLoading(null);
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setReviews((previousReviews) =>
        previousReviews.filter((review) => review._id !== id),
      );
    ...
    <div className="ReviewsManagement-container">
      ...
      <section className="ReviewsManagement-metrics">
        <article className="ReviewsManagement-metricCard">
          <div className="ReviewsManagement-metricIcon total">★</div>

          <div className="ReviewsManagement-metricInfo">
            <span className="ReviewsManagement-metricTitle">Total Reviews</span>
        ...
        <article className="ReviewsManagement-metricCard">
          <div className="ReviewsManagement-metricIcon average">★</div>
        ...
        <article className="ReviewsManagement-metricCard">
          <div className="ReviewsManagement-metricIcon published">✓</div>
        ...
        <article className="ReviewsManagement-metricCard">
          <div className="ReviewsManagement-metricIcon pending">!</div>
      ...
      <section className="ReviewsManagement-toolbar">
        ...
        <div className="ReviewsManagement-filters">
          <CustomDropdown
            ...
            onChange={(value) => handleFilterChange(setRatingFilter, value)}
          />
            ...
            {paginatedReviews.length > 0 ? (
                  ...
                  <td>
                    <div className="ReviewsManagement-customer">
                      <div
                        ...
                        style={{
                          backgroundColor: row.customer.bg,
                  ...
                  <td>
                    <div className="ReviewsManagement-product">
                      {row.product.img ? (
                        <img
                          src={
                            row.product.img.startsWith("http")
                              ? row.product.img
              ...
              <tr>
                <td colSpan="7" className="ReviewsManagement-empty">
                  <div className="ReviewsManagement-emptyIcon">⭐</div>
