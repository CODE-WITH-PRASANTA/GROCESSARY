...
const findOrCreateBrand = async (value) => {
  ...
  if (/^[0-9a-fA-F]{24}$/.test(brandValue)) {
    const brandById =
      await Brand.findById(brandValue);
...
const findOrCreateUnit = async (value) => {
  ...
  if (
    ...
    ["l", "ltr", "liter", "litre", "liters", "litres", "ml", "milliliter", "millilitre"].includes(
      normalizedUnit
    )
  ) {
    unitType = "Volume";
  ...
  try {
    ...
    for (
      ...
      try {
        ...
        if (!categoryDoc) {
          throw new Error(
            `Unable to create category "${categoryValue}"`
          );
        }
        ...
        if (brandValue) {
          ...
          if (!brandDoc) {
            throw new Error(
              `Unable to create brand "${brandValue}"`
            );
          }
        }
        ...
        if (!unitDoc) {
          throw new Error(
            `Unable to create unit "${unitValue}"`
          );
        }
      ...
      if (
        ...
        if (!category) {
          return res.status(400).json({
            success: false,
            message:
              `Unable to create category "${body.category}".`,
          });
        }
      ...
      if (
        ...
        if (!unit) {
          return res.status(400).json({
            success: false,
            message:
              `Unable to create unit "${body.unit}".`,
          });
        }
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "Status must be active, inactive, published, or unpublished.",
        });
      }
